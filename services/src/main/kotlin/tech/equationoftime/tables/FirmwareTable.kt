...
val Database.firmwares get() = this.sequenceOf(FirmwareTable)
val Database.families get() = this.sequenceOf(FirmwareFamilyTable)
...
object FirmwareTable : Table<FirmwareEntity>("t_firmware") {
    val id = int("id").primaryKey().bindTo { it.id }
    val firmwareId = varchar("firmware_id").bindTo { it.firmwareId }
    val familyId = int("family_id").references(FirmwareFamilyTable){ it.family}
...
object FirmwareFamilyTable : Table<FirmwareFamilyEntity>("t_firmware_metadata") {
    val id = int("id").primaryKey().bindTo { it.id }
